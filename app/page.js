...
export default function MiniTalabat() {
  ...
  // تجميع السلة حسب المحل للعرض ولرسالة الواتساب
  ...
  return (
    <div dir="rtl" style={{ padding: '10px', fontFamily: 'sans-serif', backgroundColor: '#121212', color: '#e0e0e0', minHeight: '100vh', paddingBottom: '100px' }}>
            ...
            <>
              {Object.keys(groupedCart).map(shopName => (
                <div key={shopName} style={{ marginBottom: '20px', border: '1px solid #333', borderRadius: '12px', overflow: 'hidden' }}>
                  <div style={{ backgroundColor: '#333', padding: '8px 15px', color: '#FF6600', fontWeight: 'bold', fontSize: '14px' }}>📍 متجر: {shopName}</div>
                    ...
                    <div key={item.key} style={{ padding: '12px', borderBottom: '1px solid #222', backgroundColor: '#1e1e1e' }}>
                      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                        ...
                        <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
                          ...
                          <button onClick={() => removeFromCart(item.key)} style={{ color: '#ff4444', background: 'none', border: '1px solid #ff4444', borderRadius: '50%', width: '22px', height: '22px', fontSize: '14px', lineHeight: '1' }}>-</button>
                        </div>
                      </div>
                      <input 
              ...
              <div style={{ backgroundColor: '#1e1e1e', padding: '15px', borderRadius: '15px', border: '1px solid #FF6600' }}>
                ...
                <button onClick={sendOrder} style={{ width: '100%', padding: '15px', backgroundColor: '#25D366', color: '#fff', border: 'none', borderRadius: '12px', marginTop: '15px', fontWeight: 'bold', fontSize: '16px' }}>إرسال الطلب للواتساب ✅</button>
              </div>
            </>
          )}
        </div>
      ...
      <nav style={{ position: 'fixed', bottom: 0, left: 0, right: 0, backgroundColor: '#1e1e1e', display: 'flex', justifyContent: 'space-around', padding: '12px 0', borderTop: '2px solid #333', zIndex: 1000 }}>
        ...
        <button onClick={() => setActiveTab('cart')} style={{ background: 'none', border: 'none', color: activeTab === 'cart' ? '#FF6600' : '#888', textAlign: 'center', position: 'relative' }}>
          ...
          {Object.keys(cart).length > 0 && (
            <span style={{ position: 'absolute', top: '-5px', right: '10px', backgroundColor: '#FF6600', color: '#fff', borderRadius: '50%', padding: '2px 6px', fontSize: '10px', fontWeight: 'bold' }}>{Object.keys(cart).length}</span>
          )}
        </button>
      </nav>
    </div>
