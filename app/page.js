...
export default function MiniTalabat() {
  ...
  // --- هنا بتتحكم في فتح وقفل المحلات (true = مفتوح / false = مغلق) ---
  const shops = [
        ...
        <div key={shop.id} style={{ border: '1px solid #333', borderRadius: '12px', padding: '12px', marginBottom: '12px', backgroundColor: '#1e1e1e', opacity: shop.isOpen ? 1 : 0.7 }}>
          ...
          {shop.items.map(item => (
            <div key={item.name} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', margin: '10px 0' }}>
              <span style={{ fontSize: '14px', color: shop.isOpen ? '#e0e0e0' : '#888' }}>{item.name} <b style={{color: '#FF6600'}}>{item.price} ج.م</b></span>
              {/* لا يظهر زر الإضافة إذا كان المحل مغلقاً */}
              {shop.isOpen && (
