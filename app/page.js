...
export default function MiniTalabat() {
  ...
  const [orderNotes, setOrderNotes] = useState(""); // مخزن للملاحظات
  ...
  return (
    <div dir="rtl" style={{ padding: '10px', fontFamily: 'sans-serif', backgroundColor: '#121212', color: '#e0e0e0', minHeight: '100vh', paddingBottom: '180px' }}>
      ...
      {filteredShops.map(shop => (
        <div key={shop.id} style={{ border: '1px solid #333', borderRadius: '12px', padding: '12px', marginBottom: '12px', backgroundColor: '#1e1e1e', opacity: shop.isOpen ? 1 : 0.7 }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '8px' }}>
            <h3 style={{ borderRight: '3px solid #FF6600', paddingRight: '8px', margin: 0, fontSize: '16px' }}>{shop.name}</h3>
            <span style={{ fontSize: '11px', padding: '2px 8px', borderRadius: '10px', backgroundColor: shop.isOpen ? '#1b5e20' : '#b71c1c', color: '#fff' }}>
      ...
      {/* السلة الذكية مع الملاحظات */}
      {calculateTotal() > 0 && (
        <div style={{ position: 'fixed', bottom: 0, left: 0, right: 0, backgroundColor: '#1e1e1e', borderTop: '2px solid #FF6600', zIndex: 2000, padding: '15px', borderRadius: '25px 25px 0 0', boxShadow: '0 -8px 20px rgba(0,0,0,0.5)' }}>
          {!showOrderForm ? (
            <div>
                ...
                {Object.keys(groupedCart).map(shopName => (
                  <div key={shopName} style={{ marginBottom: '8px', borderBottom: '1px solid #333', paddingBottom: '5px' }}>
                    ...
                    {groupedCart[shopName].map(item => (
                      <div key={item.key} style={{ display: 'flex', justifyContent: 'space-between', padding: '3px 0' }}>
              ...
              {/* خانة الملاحظات الجديدة */}
              <textarea 
                placeholder="أي ملاحظات إضافية؟ (مثلاً: بلاش بصل، العمارة الزرقاء..)" 
                value={orderNotes}
                onChange={(e) => setOrderNotes(e.target.value)}
                style={{ width: '100%', backgroundColor: '#121212', color: '#fff', border: '1px solid #444', borderRadius: '10px', padding: '8px', fontSize: '12px', marginBottom: '10px', resize: 'none', outline: 'none' }}
                rows="2"
              ></textarea>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
              ...
              <button onClick={() => setShowOrderForm(false)} style={{ color: '#aaa', background: 'none', border: 'none', fontSize: '12px' }}>تعديل الملاحظات أو السلة</button>
            </div>
          )}
        </div>
      )}
...
const inputStyle = { width: '100%', padding: '12px', borderRadius: '10px', border: '1px solid #444', backgroundColor: '#121212', color: '#fff', fontSize: '14px' };
