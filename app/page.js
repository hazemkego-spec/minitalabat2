...
export default function MiniTalabat() {
  ...
  const [activeTab, setActiveTab] = useState('home'); // التبويب النشط (home أو cart)
  ...
  const sendOrder = () => {
    ...
    Object.keys(cart).forEach(key => {
      const [shopName, itemName] = key.split('-');
      orderDetails += `\n• ${itemName} من ${shopName} (${cart[key]}ق)`;
    });
  ...
  return (
    <div dir="rtl" style={{ padding: '10px', fontFamily: 'sans-serif', backgroundColor: '#121212', color: '#e0e0e0', minHeight: '100vh', paddingBottom: '80px' }}>
          ...
          {Object.keys(cart).length === 0 ? (
            ...
            <>
              {Object.keys(cart).map(key => (
                <div key={key} style={{ display: 'flex', justifyContent: 'space-between', backgroundColor: '#1e1e1e', padding: '12px', borderRadius: '10px', marginBottom: '10px' }}>
                  <span>{key.split('-')[1]} ({cart[key]}ق)</span>
                  <button onClick={() => removeFromCart(key)} style={{ color: '#ff4444', background: 'none', border: '1px solid #ff4444', borderRadius: '5px', padding: '2px 8px' }}>إزالة</button>
                </div>
              ))}
              
              <div style={{ marginTop: '20px', backgroundColor: '#1e1e1e', padding: '15px', borderRadius: '15px' }}>
                <h4 style={{ marginBottom: '10px' }}>بيانات التوصيل:</h4>
                ...
                <div style={{ marginTop: '15px', textAlign: 'center', fontSize: '18px', fontWeight: 'bold' }}>الإجمالي: {calculateTotal()} ج.م</div>
                <button onClick={sendOrder} style={{ width: '100%', padding: '15px', backgroundColor: '#25D366', color: '#fff', border: 'none', borderRadius: '10px', marginTop: '15px', fontWeight: 'bold' }}>إرسال الطلب واتساب ✅</button>
              </div>
            </>
          )}
        </div>
      ...
      <nav style={{ position: 'fixed', bottom: 0, left: 0, right: 0, backgroundColor: '#1e1e1e', display: 'flex', justifyContent: 'space-around', padding: '12px', borderTop: '1px solid #333', zIndex: 1000 }}>
        <button onClick={() => setActiveTab('home')} style={{ background: 'none', border: 'none', color: activeTab === 'home' ? '#FF6600' : '#888', textAlign: 'center' }}>
          <div style={{ fontSize: '20px' }}>🏠</div>
          <div style={{ fontSize: '12px' }}>الرئيسية</div>
        </button>
        <button onClick={() => setActiveTab('cart')} style={{ background: 'none', border: 'none', color: activeTab === 'cart' ? '#FF6600' : '#888', textAlign: 'center', position: 'relative' }}>
          <div style={{ fontSize: '20px' }}>🛒</div>
          <div style={{ fontSize: '12px' }}>السلة</div>
          {Object.keys(cart).length > 0 && (
            <span style={{ position: 'absolute', top: '-5px', right: '5px', backgroundColor: '#ff4444', color: '#fff', borderRadius: '50%', padding: '2px 6px', fontSize: '10px' }}>{Object.keys(cart).length}</span>
          )}
        </button>
      </nav>
    </div>
...
const inputStyle = { width: '100%', padding: '12px', borderRadius: '8px', border: '1px solid #333', backgroundColor: '#121212', color: '#fff', marginBottom: '10px', outline: 'none' };
