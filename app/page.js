  ...
  return (
    <div dir="rtl" style={{ padding: '10px', fontFamily: 'sans-serif', backgroundColor: '#121212', color: '#e0e0e0', minHeight: '100vh', paddingBottom: '160px' }}>
      ...
      <header style={{ textAlign: 'center', marginBottom: '15px' }}>
        {/* اللوجو بتصميم حلو ومميز مع تأثير مضيء */}
      ...
      {/* السلة الرشيقة (Compact Cart) المعدلة */}
      {calculateTotal() > 0 && (
        <div style={{ position: 'fixed', bottom: 0, left: 0, right: 0, backgroundColor: '#1e1e1e', borderTop: '2px solid #FF6600', zIndex: 2000, padding: '15px', borderRadius: '25px 25px 0 0', boxShadow: '0 -10px 20px rgba(0,0,0,0.5)' }}>
          {!showOrderForm ? (
            <div>
              {/* قائمة مصغرة جداً لعرض المنتجات مع الملاحظات */}
              <div style={{ maxHeight: '100px', overflowY: 'auto', marginBottom: '10px' }}>
                {Object.keys(groupedCart).map(shopName => (
                  <div key={shopName} style={{ marginBottom: '8px', borderBottom: '1px solid #333', paddingBottom: '5px' }}>
                    ...
                    {/* خانة الملاحظات لكل منتج أصبحت تحت قائمة المنتجات */}
                    {groupedCart[shopName].map(item => (
                        <input 
                            key={`note-${item.key}`}
