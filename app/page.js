    ...
    <div dir="rtl" style={{ padding: '10px', fontFamily: 'sans-serif', backgroundColor: '#121212', color: '#e0e0e0', minHeight: '100vh', paddingBottom: '110px' }}>
        ...
        <>
          <header style={{ textAlign: 'center', marginBottom: '15px' }}>
            ...
            <div style={{ position: 'relative', margin: '15px 5px' }}>
              <input 
                type="text" 
                placeholder="ابحث عن محل أو منتج (بيتزا، لبن...)" 
                value={searchQuery}
          ...
          {filteredShops.length === 0 && <p style={{ textAlign: 'center', color: '#666' }}>لم نجد ما تبحث عنه للأسف 😅</p>}
        </>
      )}
                ...
                <div key={shopName} style={{ marginBottom: '20px', border: '1px solid #333', borderRadius: '12px', overflow: 'hidden' }}>
                  ...
                  {getGroupedCart()[shopName].map(item => (
                    <div key={item.key} style={{ padding: '12px', borderBottom: '1px solid #222', backgroundColor: '#1e1e1e' }}>
                      ...
                      <input placeholder="أضف ملاحظة..." value={item.note} onChange={(e) => updateItemNote(item.key, e.target.value)} style={{ width: '100%', backgroundColor: 'transparent', color: '#aaa', border: 'none', borderBottom: '1px solid #444', fontSize: '12px', marginTop: '8px' }} />
                    </div>
                  ))}
                </div>
              ))}
      ...
      {activeTab === 'addShop' && (
        <div style={{ padding: '20px', textAlign: 'center' }}>
          ...
          <div style={{ backgroundColor: '#1e1e1e', padding: '20px', borderRadius: '15px', border: '1px dashed #FF6600' }}>
            <h3 style={{ fontSize: '16px', marginBottom: '15px' }}>ابدأ رحلتك الآن</h3>
            <button 
              onClick={() => window.open(`https://wa.me/${MAIN_PHONE}?text=${encodeURIComponent("أهلاً، حابب أعرف تفاصيل إزاي أضيف محلي في ميني طلبات")}`)} 
              style={{ width: '100%', padding: '15px', backgroundColor: '#FF6600', color: '#fff', border: 'none', borderRadius: '12px', fontWeight: 'bold' }}
            >
              تواصل معنا واتساب 📲
            </button>
            <p style={{ fontSize: '11px', marginTop: '10px', color: '#777' }}>سيتم الرد عليك في أسرع وقت ممكن</p>
          </div>
        </div>
      )}
