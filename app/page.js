...
export default function MiniTalabat() {
  ...
  const shops = [
    ...
    { id: 3, category: "صيدليات", name: "صيدلية الشفاء", isOpen: false, items: [{ name: "بندول", price: 30 }] },
  ...
  const sendOrder = () => {
    if(!customerInfo.name || !customerInfo.phone || !customerInfo.address) return alert("برجاء ملء البيانات");
  ...
  return (
    <div dir="rtl" style={{ padding: '10px', fontFamily: 'sans-serif', backgroundColor: '#121212', color: '#e0e0e0', minHeight: '100vh', paddingBottom: '160px' }}>
      ...
      <header style={{ textAlign: 'center', marginBottom: '15px' }}>
        {/* تم تحديث مسار اللوجو للاسم الجديد هنا لضمان ظهوره */}
        <img src="/mall-logo.png" alt="Logo" style={{ width: '70px', height: '70px', borderRadius: '50%', marginBottom: '5px', filter: 'drop-shadow(0 0 8px #FF6600)', objectFit: 'contain' }} />
        <h1 style={{ color: '#FF6600', margin: '5px 0', fontSize: '24px' }}>Mini Talabat</h1>
        <p style={{ fontSize: '14px', color: '#aaa', margin: '0 0 10px 0' }}>أكبر مول تجاري رقمي في جيبك</p>
        <button onClick={() => window.open(`https://wa.me/${MAIN_PHONE}?text=${encodeURIComponent("محتاج أضيف محلي")}`)} style={{ backgroundColor: '#1e1e1e', color: '#fff', border: '1px solid #FF6600', padding: '6px 15px', borderRadius: '15px', fontSize: '12px' }}>🧡 ضيف محلك معنا</button>
      </header>

      {/* الأقسام */}
      <div style={{ display: 'flex', overflowX: 'auto', gap: '8px', padding: '10px 0', marginBottom: '15px', whiteSpace: 'nowrap', scrollbarWidth: 'none' }}>
        {categories.map(cat => (
          <button key={cat} onClick={() => setActiveCategory(cat)} style={{ padding: '8px 20px', borderRadius: '20px', border: activeCategory === cat ? 'none' : '1px solid #333', backgroundColor: activeCategory === cat ? '#FF6600' : '#1e1e1e', color: activeCategory === cat ? '#fff' : '#bbb' }}>{cat}</button>
        ))}
      </div>

      {/* المحلات */}
      {filteredShops.map(shop => (
        <div key={shop.id} style={{ border: '1px solid #333', borderRadius: '15px', padding: '15px', marginBottom: '15px', backgroundColor: '#1e1e1e', opacity: shop.isOpen ? 1 : 0.7 }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '10px' }}>
            <h3 style={{ borderRight: '4px solid #FF6600', paddingRight: '10px', margin: 0 }}>{shop.name}</h3>
            <span style={{ fontSize: '11px', padding: '4px 10px', borderRadius: '12px', backgroundColor: shop.isOpen ? '#1b5e20' : '#b71c1c', color: '#fff', fontWeight: 'bold' }}>
      ...
      {/* السلة */}
      {calculateTotal() > 0 && (
        <div style={{ position: 'fixed', bottom: 0, left: 0, right: 0, backgroundColor: '#1e1e1e', borderTop: '2px solid #FF6600', zIndex: 2000, padding: '15px', borderRadius: '25px 25px 0 0', boxShadow: '0 -10px 20px rgba(0,0,0,0.5)' }}>
          {!showOrderForm ? (
            <div>
                    ...
                    {groupedCart[shopName].map(item => (
                      <div key={item.key} style={{ marginBottom: '5px' }}>
                        ...
                        <input 
                          ...
                          style={{ width: '100%', backgroundColor: '#121212', color: '#999', border: 'none', borderBottom: '1px solid #333', fontSize: '10px', padding: '2px 0', outline: 'none', marginTop: '2px' }}
                        />
                      </div>
                    ))}
                  </div>
              ...
              <button onClick={() => setShowOrderForm(true)} style={{ width: '100%', padding: '12px', backgroundColor: '#FF6600', color: '#fff', border: 'none', borderRadius: '12px', fontWeight: 'bold', fontSize: '16px' }}>
